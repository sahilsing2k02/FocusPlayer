        ...
        <div className="todo-list-container" style={{ display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '8px', maxHeight: '250px', overflowY: 'auto', paddingRight: '4px' }}>
                {todos.length === 0 ? (
                    ...
                    todos.map(todo => (
                        <div key={todo.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: 'var(--c-overlay)', padding: '12px 16px', borderRadius: '12px', border: '1px solid rgba(239,68,68,0.1)', animation: 'slideInRight 0.4s cubic-bezier(0.16, 1, 0.3, 1)', transition: 'all 0.2s' }} onMouseEnter={(e) => { e.currentTarget.style.transform = 'scale(1.02)'; e.currentTarget.style.borderColor = 'var(--c-border-strong)'; }} onMouseLeave={(e) => { e.currentTarget.style.transform = 'scale(1)'; e.currentTarget.style.borderColor = 'var(--c-overlay)'; }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: '12px' }}>
                                <div onClick={() => toggleTodo(todo.id)} style={{ width: '20px', height: '20px', borderRadius: '50%', border: `2px solid ${todo.done ? 'var(--success-color)' : 'var(--c-overlay)'}`, background: todo.done ? 'var(--success-color)' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer', transition: 'all 0.2s' }}>
                            ...
                            <button onClick={() => deleteTodo(todo.id)} style={{ background: 'var(--c-overlay)', border: 'none', color: '#fca5a5', width: '28px', height: '28px', borderRadius: '8px', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 0.2s' }} title="Delete task" onMouseEnter={(e) => { e.currentTarget.style.background = 'var(--c-hover-solid)'; e.currentTarget.style.color = 'white'; }} onMouseLeave={(e) => { e.currentTarget.style.background = 'var(--c-overlay)'; e.currentTarget.style.color = '#fca5a5'; }}>
